...
class FavouriteCreateList(APITestCase):
    ...
    def setUp(self):
        ...
        self.post = Article.objects.create(title="Başlık", content="İçerik")
    ...
    def test_jwt_authentication(self):
        ...
        self.assertTrue("access" in json.loads(response.content))  #bunun icinde access varmı demek
...
class FavouriteUpdateDelete(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("favourite:update-delete", kwargs={"pk": self.favourite.pk}) # olustuguran facourite linkine gtmek icin
        self.test_jwt_authentication()
    ...
    def test_fav_delete(self):
        response = self.client.delete(self.url)  #delete butonuna basmann altında bu var
        self.assertEqual(204, response.status_code)  #content yok demek silmede geriye bir şey donmuyor cunku
    # başkası benim yerime silememeli

    def test_fav_delete_different_user(self):  #baska biri bana ait biseyle işlem yapablirmi
